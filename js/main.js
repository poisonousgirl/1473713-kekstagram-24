...
const GET_UNIC_ARRAY_INT = (array) => {
  const random = _.random(0, array.length - 1);
...
const createCommentsData = () => {
  ...
  const unicAvatar = avatarArray[_.random(0, avatarArray.length - 1)];
  return {
    id: unicID[0],
...
const createPhotoData = () => {
  ...
  const commentsGen = Array.from({length: commentsArray[_.random(0, commentsArray.length - 1)]}, createCommentsData);
  return {
    id: unicID[0],
